		...
		<table >
			...
			<tr>
				<td colspan="2"><a href="#" id="AddMoreFileBox"
					class="btn btn-info">增加箱号</a></td>
			</tr>

		</table>
		<form id="myForm" action="test.action?methodstr=execute2"
					...
					<td>
						<div class="uploader
							...
							<input type="text" class="filename" readonly="readonly" /> <input
								...
								type="file" name="excel" id="excel1" size="30" /> <input
								type="hidden" name="workId" value="<%=path%>" id="sss" />
						</div>
					</td>
				</tr>
			
				<tr>
					qq<td><button class="btn btn-primary" id="ajaxA" value="ss">提交</button></td>
				</tr>
			</table>
		</form>
	</div>
	...
	$(document).ready(function() {
		...
		$(AddButton).click(function(e) //on add input button click  
		{
			if (x <= MaxInputs) //max input box allowed  
			{
				...
				$(sample).append(
"<tr><td><div class='uploader blue'><input type='text' class='filename' readonly='readonly' /> <input type='button' name='file' class='button' value='浏览...' /><input 	type='file' name='excel' id=" + FieldCount + " size='30' /> <input type='hidden' name='workId'  value=''  id='sss' /></div><a href='#' class='removeclass'>x</a></td></tr>");
