...
<body>
	<form id="myForm" action="<%=path%>/re.action?methodstr=querySend"
		method="post">
		卷轴号查检查单： <input type="text" name="barcode" /> <input type="submit" value="查询" class="btn btn-primary" />
	</form>
	<div>
		...
		沒有查到結果 
			...
			<c:forEach items="${requestScope.query }" var="bath">
				...
				<tr>
					...
					<td>${bath.sendTime}</td>
					<td><form id="myForm"
							action="<%=path%>/re.action?methodstr=sendOut" method="post">
							<input type="hidden" value="${bath.odd} " name="inspection" /> <input
								type="submit" class="btn btn-primary" id="" value="送出检查单" />
						</form></td>
				</tr>
			</c:forEach>
		</table>
