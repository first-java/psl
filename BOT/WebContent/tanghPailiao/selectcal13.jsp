		...
		$(document).ready(function() {
			$("#ajaxA").click(function() {
				...
				$.ajax({
					...
					type : "POST",
					//提交的网址
					url : "csppailiao.action?methodstr=select13", //11.2selectShelft
					//提交的数据
					/* data : {
					...
					success : function(result) {
						...
						for (var i = 0; i < result.counter.length; i++) {
							...
							var cntargs1 = result.counter[i].cntargs1;
							...
							var bin1 = result.counter[i].bin1;
							var pianQty = result.counter[i].pianQty;
							...
							var zong = Number(chayi) + Number(mesQty) + Number(bin1) + Number(pianQty) + Number(zhQty) + Number(cntargs1) + Number(mesQty);
							var zai = Number(qty) - Number(zong)
							s = "<tr name='sss'><td>" + fgLotId + "</td><td>" +
								adtime + "</td><td>" +
								lotType + "</td><td>" +
								id + "</td><td>" +
								qty + "</td><td>" +
