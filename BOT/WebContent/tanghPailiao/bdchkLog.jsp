	...
	<table>
		...
		<tr>
			<td colspan="4">选择<select name="leixing"><option>日期类型...</option>
				...
				onclick="downLoad('cspdaoru.action?methodstr=downloadBDchkLog')"
		...
		$(document).ready(function() {
			$("#ajaxA").click(function() {
				...
				$.ajax({
					...
					type : "POST",
					//提交的网址
					url : "csppailiao.action?methodstr=bdchkLog", //11.2selectShelft
					//提交的数据
					data : {
						"eqpt.startTime" : "0",
					...
					success : function(result) {
						...
						for (var i = 0; i < result.counter.length; i++) {
							...
							var errorMsg = result.counter[i].errorMsg;
							var lotId = result.counter[i].lotId;
							var odd = result.counter[i].odd;
							$("#page").val(Number(odd));
							var objectType = result.counter[i].objectType;
							var createTime = result.counter[i].createTime;
							var objectId = result.counter[i].objectId
							var xiangmu = result.counter[i].xiangmu
							var adtime = "";
							if (createTime != null) {
							...
							s = "<tr name='sss'><td>" + odd + "</td><td>" + eqpt + "</td><td>" + errorMsg + "----" + objectId + "</td><td>" +
								lotId + "</td><td>" +
								objectType + "</td><td>" +
								objectId + "</td><td>" +
