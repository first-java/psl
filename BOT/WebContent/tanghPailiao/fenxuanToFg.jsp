	...
	<div class="page-header">
		<table align="left">
			<tr>
				...
				<td id="tt">分选底板号：<input id="lotId" type="text"
...
<script>
	...
	$(document).ready(function() {
		...
		$("#ajaxA").click(function() {
			...
			$.ajax({
				...
				type : "POST",
				//提交的网址
				url : "csppailiao.action?methodstr=fenxuanToFg", //5.1
				//提交的数据
				data : {
					"putIn.lotId" : lotId
				},
				...
				success : function(result) {
					...
					for (var i = 0; i < result.counter.length; i++) {
						var barcode = result.counter[i].binUnitId;
						var id = result.counter[i].id;
						var qty = result.counter[i].qty;

						s = "<tr name='sss'><td>" +
							"<input type='hidden' name='lotId' value=" + barcode + " />" + barcode + "</td><td>" +
		...
		$("#ajaxB").click(function() {
			...
			$.ajax({
				...
				type : "POST",
				//提交的网址
				url : "csppailiao.action?methodstr=fenxuanToFg1", //5.1
				//提交的数据
				data : {
					"putIn.lotId" : lotId,
					"putIn.seri" : seri,
	...
	$(document).ready(function() {
		...
		$("#printTplBtn").on("click", function() {
			...
			printparamsJsonArray.push({
				"PTK_DrawTextEx" : "20,45,0,3,2,1,N,分选底板转分光入库,0"
			}); //打印文字
