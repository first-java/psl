...
%>
<%
    String lotId = (String) request.getParameter("lotId");//直接传递
...
<body onload="check();">
	...
	<input class="btn btn-primary"
		onclick="downLoad('cspdaoru.action?methodstr=downloadDFGmingxi2')"
		value="导出清单">
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			$("#ajaxA").click(function() {
				...
				$.ajax({
					...
					type : "POST",
					//提交的网址
					url : "csppailiao.action?methodstr=selDFGmingxi2", //11.2selectShelft
					//提交的数据
					data : {
						"putIn.lotId" : lotId,
						"putIn.status" : status
					...
					success : function(result) {
						...
						for (var i = 0; i < result.counter.length; i++) {
							...
							var qty = result.counter[i].mesQty;
							var deposit = result.counter[i].leiXing;
							var createTime = result.counter[i].createTime;
	
							var flag = result.counter[i].flag;
	
							if (deposit == '卷轴转分光' || deposit == '蓝膜转分光' || deposit == '分选底板转分光') {
								qty = result.counter[i].qty;
							}
							...
							s = "<tr name='sss'><td>" + bagName + "</td><td>" +
								lotId + "</td><td>" +
								qty + "</td><td>" +
								deposit + "</td><td>" +
								adtime + "</td><td>" +
		...
		function downLoad(url) {
			...
			//2、拼装URL+查询参数
			url = url + "&startTime=" + lotId + "&endTime=" + status;
