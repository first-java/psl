...
<script>
	$(document).ready(function() {
		$("#ajaxA").click(function() {
			...
			var floors = $("#shelfFloors").prop("value");
			...
			$.ajax({
				...
				type : "POST",
				//提交的网址
				url : "csppailiao.action?methodstr=saveShelf1", //1.1
				//提交的数据
				data : {
					"fgShelf.caseNo" : caseNo,
					"fgShelf.shelfName" : shelfName,
					"fgShelf.workId" : workId,
					"fgShelf.shelfFloors" : floors,
					"fgShelf.cntargs" : cntargs,
					'fgShelf.checkbox' : checkbox
				},
				//返回数据的格式
...
<body onKeyDown="_b()">
	...
	<table id="case">
		...
		<tr id="dis" style="display: none;">
			<td colspan="2">当前货架已有<span id="f"></span>个框子,<span id="cas"></span>，正在新建第<span
				id="d"></span>个
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>框 &nbsp;&nbsp;号：<input type="hidden" value="1" id="s"
...
<script>
	...
	$("#shelfName").blur(function() {
		...
		$.ajax({
			...
			type : "POST",
			//提交的网址
			url : "csppailiao.action?methodstr=selectShelf1", //12.4
			//提交的数据
			data : {
				"fgShelf.shelfName" : shelfName
			},
