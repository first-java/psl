		...
		<table align="left">
			<tr>
				<input id="clickMe" type="hidden" onclick="setFocus()"
					value="Set focus" />
				<input type="hidden" id="shelfName" value="">
				<input type="hidden" id="bag" value="">
				<input type="hidden" id="bagweight" value="">
				<input type="hidden" id="bagqty" value="">
				<td id="tt">蓝膜号：<input id="lotId" type="text"
...
<script>
	...
	$(document).ready(function() {
		...
		$("#ajaxA").click(function() {
			...
			$.ajax({
				...
				type : "POST",
				//提交的网址
				url : "csppailiao.action?methodstr=binUnitIdToFg", //5.1
				//提交的数据
				data : {
					"putIn.lotId" : lotId.trim()
				},
				//返回数据的格式
				...
				success : function(result) {
					...
					var lotHead1 = result.counter[0].lotHead;
					for (var i = 0; i < result.counter.length; i++) {
						...
						s = "<tr name='sss'><td>" +
							...
							"<input type='hidden' name='binUnitId' id='binUnitId' value=" + binUnitId + " />" + binUnitId + "</td><td>" +
						...
						if (luckElements.length > 0) {
							rr = luckElements[0].value.substring(0, 7)
							if (rr != lotHead1) {
		...
		$("#ajaxB").click(function() {
			...
			$.ajax({
				...
				type : "POST",
				//提交的网址
				url : "csppailiao.action?methodstr=binUnitIdToFg1", //5.1
				//提交的数据
				data : {
					"putIn.lotHead" : lotHead,
					"putIn.seri" : seri,
					"putIn.remark" : remark,
					"putIn.binUnitId" : binUnitId,
					"putIn.workId" : workId,
					"putIn.weight" : weight
				},
				//返回数据的格式
	...
	$(document).ready(function() {
		...
		$("#printTplBtn").on("click", function() {
			...
			printparamsJsonArray.push({
				"PTK_DrawTextEx" : "20,45,0,3,2,1,N,蓝膜转分光入库,0"
			}); //打印文字
