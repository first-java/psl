					...
					<section class="panel bg-white no-b fadeIn animated"
						...
						<div class="form-group col-sm-10"
							...
							<form id="myForm" action="cspdaoru.action?methodstr=execute2"
								...
								<table>
									<input type="hidden" name="biaoji" value="分光机台产品维护表">
									<tr>
							...
							<form id="myForm2"
								action="cspdaoru.action?methodstr=downloadMuBanEqpt"
								method="post">
								<tr>
	...
	<script type="text/javascript">
		$("#product1").blur(function() {
			...
			$.ajax({
				...
				type : "POST",
				//提交的网址
				url : "csppailiao.action?methodstr=selectQty", //9.2
				//提交的数据
				data : {
					"calculate.lotType" : lotType
				},
				//返回数据的格式
		...
		$(document).ready(function() {
			...
			$("#ajaxC").click(function() {
				...
				$.ajax({
					...
					type : "POST",
					//提交的网址
					url : "csppailiao.action?methodstr=selectPro", //产品
					/* data : {
						"eqpt.status" : "1"
					}, */
					//返回数据的格式
			...
			$("#ajaxD").click(function() {
				...
				$.ajax({
					...
					type : "POST",
					//提交的网址
					url : "csppailiao.action?methodstr=selectEqpt", //机台
					data : {
						"eqpt.status" : "1"
					},
					//返回数据的格式
			...
			$("#ajaxA").click(function() {
				...
				$.ajax({
					...
					type : "POST",
					//提交的网址
					url : "csppailiao.action?methodstr=printFg1", //1.1
					//提交的数据
					data : {
						"eqpt.eqpt" : eqpt,
...
<script>$(function() {
		...
		$("#excel3").each(function() {
			if ($(this).val() == "") {
				$(this).parents(".uploader").find(".filename").val("请选择《分光机台产品维护表》");
			}
		});
	});
