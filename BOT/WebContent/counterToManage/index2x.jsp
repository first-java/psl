	...
	$(document).ready(function() {
		$("#ajaxA").click(function() {
			...
			$.ajax({
				...
				type : "POST",
				//提交的网址
				url : "ctm.action?methodstr=putIn1", //9.1
				//提交的数据
				data : {
		...
		$("#ajaxF").click(function() {
			...
			$.ajax({
				...
				type : "POST",
				//提交的网址
				url : "ctm.action?methodstr=selectPutIn1", //9.2
				//提交的数据
				data : {
					"counter.product" : opt,
					"counter.status" : "1"
		...
		<table id="sample-table-1">
			...
			<tr>
				<td><input id="clickMe" type="hidden" onclick="setFocus()"
					value="Set focus" /> 输入蓝膜号 ： <input type="text" id="product"
					size="16" name="product" style="text-transform: uppercase;"
					onkeypress='if(event.keyCode==13) yy();' /></td>
				<td id="nameDiv" style="display:none"><p
						style="color:red;font-size:16px;">输入的信息未找到！</p></td>
				<td id="nameDiv1" style="display:none"><p
						style="color:red;font-size:16px;">请输入蓝膜或报检单号！</p></td>
				<input type="hidden" value="<%=workId%>" id="workId" />
				<input type="hidden" id="ajaxF" />
			</tr>
		</table>
