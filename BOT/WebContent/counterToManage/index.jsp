...
<body onKeyDown="_b()">
	...
	<table id="case">
		<tr>
			...
			<td colspan="2">新建货架名称：<input type="text" id="shelfName" size="10" /></td>
			<td id="nameDiv1" style="display:none"><p style="color:red;font-size:16px;">货架名称不能为空！</p></td>
		</tr>
		<tr>
			<td>&nbsp;</td>
		</tr>
		<tr id="dis" style="display: none;">
			<td colspan="2">当前货架已有<span id="f"></span>层数,正在新建第<span id="d"></span>层
			</td>
		</tr>
		<tr>
			<td>&nbsp;</td>
		...
		<tr>
			...
			<td>箱 &nbsp;&nbsp;号：<input type="hidden" value="1" id="s" name="cntargs" /> 
				<input type="text" id="caseNo" size="10" name="caseNo" /></td>
...
<script>
	...
	$("#shelfName").blur(function() {
		...
		$.ajax({
			...
			type : "POST",
			//提交的网址
			url : "ctm.action?methodstr=zLook4", //12.4
			//提交的数据
			data : {
				"counter.shelfName" : shelfName
			},
			...
			success : function(result) {
				...
				s = "<td name='sss'>" + product + "</td>";
				e = "<td name='sss'>" + (parseInt(product) + 1) + "</td>";
