...
<script>
	$(document).ready(function() {
		$("#ajaxS").click(function() {
			layer.load();
			var lotHead = $('#lotHead1').val();
			var qty = $('#lotHead2').val();
			var materialName = $('#lotHead3').val();
			if (lotHead == '' || qty == '') {
				layer.msg('输入框不能为空！', {
					icon : 2
				}, function() {
			...
			$.ajax({
				...
				type : "POST",
				//提交的网址
				url : "ctm.action?methodstr=filmNoBind", //12.3
				//提交的数据
				data : {
...
<body>
	<table align="left" id="sample-table-1">
		<tr>
			<td>输入两位或五位批次头：<input type="text" id="lotHead1"
				onkeypress='if(event.keyCode==13) t(this,1);'
				onkeyup="this.value=this.value.toUpperCase()" maxlength="5" /></td>
			<td>输入满卷数量: <input type="text" id="lotHead2"
				onkeypress='if(event.keyCode==13) t(this,2);'
				onChange="shuru(this);" oninput="if(value>999)value=1000 "></td>
			<td>输入级别: <input type="text" id="lotHead3"
				onkeypress='if(event.keyCode==13) setFocus();' /></td>
			<td colspan="2">
