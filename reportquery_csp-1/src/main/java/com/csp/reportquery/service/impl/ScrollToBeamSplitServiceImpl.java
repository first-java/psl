...
import com.csp.reportquery.dao.ScrollToBeamSplitDao;
 ...
 */
@Service
public class ScrollToBeamSplitServiceImpl implements ScrollToBeamSplitService {

	@Autowired
	ScrollToBeamSplitDao scrollToBeamSplitDao;

	@Override
	public List<ReportBagHe> queryScrollToBeamSplit(int page, int limit, String keyWord) {
		int page1 = (page - 1) * limit + 1;
		int limit1 = page * limit;
		return scrollToBeamSplitDao.queryScrollToBeamSplit(page1, limit1, keyWord);
	}

	@Override
	public Integer queryAllCount(String keyWord) {

		return scrollToBeamSplitDao.queryAllCount(keyWord);
	}

	@Override
	public List<Map<String, Object>> queryScrollToBeamSplitByTime(String startTime, String endTime) {

		return scrollToBeamSplitDao.queryScrollToBeamSplitByTime(startTime, endTime);
	}
	...
	private void exportExcel(HSSFWorkbook workbook, String startTime, String endTime) throws Exception {
		...
		List<Map<String, Object>> listMap = queryScrollToBeamSplitByTime(startTime, endTime);
		...
		for (int i = 0; i < listMap.size(); i++) {
			...
			row.createCell(3)
					.setCellValue(listMap.get(i).get("BARCODE") == null ? "" : listMap.get(i).get("BARCODE").toString());
