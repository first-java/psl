...
import com.csp.reportquery.dao.FgLotIdDataDao;
import com.csp.reportquery.entity.ReportBagHe;
import com.csp.reportquery.service.FgLotIdDataService;

/**
 * @ClassNmame FgLotIdDataServiceImpl.java
 * @author 奥特虾不会写代码
 * @author E-mail:pengshiliang@latticepower.com
 * @version 1.0
 * @time 创建时间：2020年4月18日 下午4:37:16
 * @Copyright © 2020 by 奥特虾不会写代码
 */
@Service
public class FgLotIdDataServiceImpl implements FgLotIdDataService {

	@Autowired
	FgLotIdDataDao fgLotIdDataDao;

	@Override
	public List<ReportBagHe> queryFgLotIdData(int page, int limit, String keyWord) {
		int page1 = (page - 1) * limit + 1;
		int limit1 = page * limit;
		return fgLotIdDataDao.queryFgLotIdData(page1, limit1, keyWord);
	}

	@Override
	public Integer queryAllCount(String keyWord) {

		return fgLotIdDataDao.queryAllCount(keyWord);
	}

	@Override
	public List<Map<String, Object>> queryFgLotIdDataByTime(String startTime, String endTime) {

		return fgLotIdDataDao.queryFgLotIdDataByTime(startTime, endTime);
	}
	...
	public void salaryTemplate(HttpServletResponse response, String startTime, String endTime) throws Exception {
		...
		response.setHeader("Content-Disposition",
				"attachment;filename=" + new String("分光批次号查询".getBytes("UTF-8"), "ISO-8859-1") + ".xls");
	...
	private void exportExcel(HSSFWorkbook workbook, String startTime, String endTime) throws Exception {
		// 创建创建sheet
		HSSFSheet sheet = workbook.createSheet("分光批次号查询");
		...
		headerRow.createCell(3).setCellValue("卷轴号");
		...
		headerRow.createCell(13).setCellValue("重量转换数量g");
		...
		List<Map<String, Object>> listMap = queryFgLotIdDataByTime(startTime, endTime);
		...
		for (int i = 0; i < listMap.size(); i++) {
			...
			row.createCell(3).setCellValue(
					listMap.get(i).get("BARCODE") == null ? "" : listMap.get(i).get("BARCODE").toString());
